...
export default async function getReadableTitleFromAddress(
  props: AddressPageMetadataProps
): Promise<string> {
  const {
    params: { address },
  ...
  try {
    ...
    //     return address;
    // }
    const tokenDisplayAddress =
      address.slice(0, 2) + '\u2026' + address.slice(-2);
    return `Token | ${address} (${tokenDisplayAddress})`;
  } catch {
    return address;
  }
}
