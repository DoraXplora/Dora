...
export function instructionIsSelfCPI(ixData: Buffer): boolean {
  return ixData.subarray(0, 8).equals(ANCHOR_SELF_CPI_TAG);
}

export function getAnchorProgramName(
  program: Program | null
): string | undefined {
  return program && 'name' in program.idl.metadata
    ? snakeToTitleCase(program.idl.metadata.name)
    : undefined;
}
...
function getFieldDef(
  ...
  if (typeof fields[0] === 'string') {
    return (fields as IdlType[]).find(
      (ixDefArg, argIndex) => argIndex === index
    );
  } else {
    return (fields as IdlField[]).find((ixDefArg) => ixDefArg.name === key)
      ?.type;
  }
}
...
export function mapAccountToRows(
  ...
  return Object.entries(accountData).map(([key, value], index) => {
    try {
      ...
      const fieldDef: IdlType | undefined = getFieldDef(
