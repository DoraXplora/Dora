  ...
  switch (action.type) {
    case PerformanceInfoActionType.SetPerfSamples: {
      ...
      const short = action.data
        ...
        .map((sample) => {
          return Number(
            sample.numTransactions / BigInt(sample.samplePeriodSecs)
          );
        });
      ...
      const trueShort = action.data
        .filter((sample) => {
          return sample.numNonVoteTransactions !== BigInt(0);
        })
        .map((sample) => {
          return Number(
            sample.numNonVoteTransactions / BigInt(sample.samplePeriodSecs)
          );
        });

      const trueMedium = downsampleByFactor(short, 4);
      const trueLong = downsampleByFactor(medium, 3);

      const truePerfHistory = {
        long: round(trueLong.slice(0, 30)).reverse(),
        medium: round(trueMedium.slice(0, 30)).reverse(),
        short: round(trueShort.slice(0, 30)).reverse(),
      };

      const historyMaxTps = Math.max(
        Math.max(...truePerfHistory.short),
        Math.max(...truePerfHistory.medium),
        Math.max(...truePerfHistory.long)
      );

      const status =
        state.transactionCount !== BigInt(0)
