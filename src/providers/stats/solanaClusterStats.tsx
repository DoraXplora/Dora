...
export function SolanaClusterStatsProvider({ children }: Props) {
  ...
  React.useEffect(() => {
    ...
    const getPerformanceSamples = async () => {
      try {
        const samplesResponse = await rpc
          .getRecentPerformanceSamples(60 * SAMPLE_HISTORY_HOURS)
          .send();

        const samples: PerformanceSample[] = samplesResponse.map(
          (s: {
            numSlots: bigint;
    ...
    const getTransactionCount = async () => {
      try {
        const transactionCount = await rpc
          .getTransactionCount({ commitment: 'confirmed' })
          .send();
        if (stale) {
          return;
        }
  ...
  return (
    <StatsProviderContext.Provider
      value={{ active, retry, setActive, setTimedOut }}
    >
