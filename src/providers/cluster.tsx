...
import { localStorageIsAvailable } from '@/src/utils/local-storage';
import {
...
const ModalContext = createContext<[boolean, SetShowModal] | undefined>(
  undefined
);
...
export function ClusterProvider({ children }: ClusterProviderProps) {
  ...
  const customUrl =
    (enableCustomUrl && searchParams?.get('customUrl')) || state.customUrl;
