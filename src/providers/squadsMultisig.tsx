...
export type MinimalMultisigInfo =
  | {
      version: 'v4';
      multisig: { threshold: number; members: Array<{ key: PublicKey }> };
    }
  | { version: 'v3'; multisig: { threshold: number; keys: Array<PublicKey> } };

const SQUADS_MAP_URL =
  'https://4fnetmviidiqkjzenwxe66vgoa0soerr.lambda-url.us-east-1.on.aws/isSquadV2';

// Squads Multisig reverse map info is only available on mainnet
export function useSquadsMultisigLookup(
  programAuthority: PublicKey | null | undefined,
  cluster: Cluster
) {
  return useSWRImmutable<SquadsMultisigMapInfo | null>(
    ['squadsReverseMap', programAuthority?.toString(), cluster],
    async ([_prefix, programIdString, cluster]: [
      string,
      string | undefined,
      Cluster
    ]) => {
...
export function useSquadsMultisig(
  ...
  return useSWRImmutable<MinimalMultisigInfo | null>(
    ...
    async ([_prefix, multisig, cluster]: [
      string,
      string | undefined,
      Cluster
    ]) => {
      ...
      if (version === 'v4') {
        const multisigInfo = await (
          anchorProgram.account as any
        ).multisig.fetch(multisig, 'confirmed');
        return {
          version: 'v4' as const,
          multisig: {
        ...
        const multisigInfo = await (anchorProgram.account as any).ms.fetch(
          multisig,
          'confirmed'
        );
        return {
